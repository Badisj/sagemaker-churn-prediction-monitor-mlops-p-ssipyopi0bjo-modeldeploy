...
def extend_config(args, model_package_arn, stage_config):
    ...
    candidate_model_name = f"Model-{args.sagemaker_project_name[:-6]}-{stage_name}-{args.deployment_strategy}-{timestamp}"
    endpoint_config_name = f"EndpointConfig-{args.sagemaker_project_name[:-6]}-{stage_name}-{timestamp}"
    endpoint_name = f"Endpoint-{args.sagemaker_project_name[:-6]}-{stage_name}"
